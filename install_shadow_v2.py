...
def build_dungeon_py() -> str:
    return dedent(
        ...
        def _copy_exercise(exercise_path: Path) -> None:
            shutil.copytree(exercise_path, SANDBOX_DIR, dirs_exist_ok=True)


        def _chmod_scripts() -> None:
            for path in SANDBOX_DIR.rglob("*.sh"):
        ...
        def enter_gate(gate_path: str | Path) -> tuple[int, str]:
            ...
            if not source.exists():
                return 1, f"Gate path not found: {source}"
            ...
            if not os.path.exists(tester):
                raise FileNotFoundError(f"tester.sh missing in sandbox: {tester}")
...
def build_main_py() -> str:
    return dedent(
        ...
        def main() -> None:
            ...
            if not gates:
                CONSOLE.print("No gates discovered. Ensure .subjects is present.")
                return

            choice = input("Select Gate ID: ").strip()
            if not choice.isdigit():
                CONSOLE.print(f"Invalid selection: {choice}")
                return
            index = int(choice) - 1
            if index < 0 or index >= len(gates):
                CONSOLE.print(f"Selection out of range: {choice}")
                return
            gate = gates[index]
            ...
            save_settings(
                {
                    ...
                    "system_version": settings.get("system_version", "v2.0"),
                }
            )
...
def remove_old_files() -> None:
    targets = [
        PROJECT_ROOT / "core/hunter.py",
        PROJECT_ROOT / "core/gate_loader.py",
        PROJECT_ROOT / "systems/dungeon.py",
        PROJECT_ROOT / "systems/penalty.py",
        PROJECT_ROOT / "systems/punishment.c",
        PROJECT_ROOT / "ui/hud.py",
...
def install_structure() -> None:
    ...
    (PROJECT_ROOT / "systems").mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ensure_rich()
    install_structure()
    print("SHADOW_TERMINAL v2 installed.")
